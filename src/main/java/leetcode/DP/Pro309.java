 ...
 * 3.状态转移：
 * dp[i][k][0] = Math.max(dp[i - 1][k][0](昨天未持有 - 今天不操作 - 今天不持有), dp[i - 1][k][1] + price[i](昨天持有 -> 今天卖出 -> 今天未持有）);
 * dp[i][k][1] = Math.max(dp[i - 2][k - 1][0] - price[i](昨天未持有 -> 今天买入 -> 今天持有), dp[i - 1][k][1](昨天持有 -> 今天不操作 -> 今天持有));
...
public class Pro309 {
    ...
    public int maxProfit(int[] prices) {
        ...
        for(int i = 0; i < n; i++) {
             ...
             * Cooldown:
             */
            dp[i][1] = Math.max(i >= 2 ?  dp[i - 2][0] - prices[i] : dp[i - 1][0] - prices[i], dp[i - 1][1]);
    ...
    public static void main(String[] args) {
        Pro309 pro = new Pro309();
        int[] prices = new int[]{1,2,3,0,2};
//        int[] prices = new int[]{7,1,5,3,6,4};
//        int[] prices = new int[]{7,6,4,3,1};
        System.out.println(pro.maxProfit(prices));
    }
}
